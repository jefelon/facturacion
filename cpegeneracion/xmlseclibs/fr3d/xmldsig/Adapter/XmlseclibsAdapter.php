...
class XmlseclibsAdapter implements AdapterInterface
{
    ...
    protected $idsign='signeder';

    public function __construct($idsign='signeder')
    {
        $this->idsign=$idsign;
    }
    ...
    public function sign(DOMDocument $data, $node="")
    {
        ...
        if($data->getElementsByTagName("ExtensionContent")->item(1) == null){
            $nodep = $data->getElementsByTagName("ExtensionContent")->item(0);
        }else{
            $nodep = $data->getElementsByTagName("ExtensionContent")->item(1);
        }
        ...
        //$objXMLSecDSig->addReference($node, $this->digestAlgorithm, $this->transforms, ['force_uri' => true]);
        ...
        $objXMLSecDSig->sign($objKey, $nodep);


        /* Add associated public key */
        if ($this->getPublicKey()) {
            $objXMLSecDSig->add509Cert($this->getPublicKey(), true, false, array('subjectName' => TRUE));
        }
    }
    ...
    {
        return new XMLSecurityDSig('ds', $this->idsign);
    }
