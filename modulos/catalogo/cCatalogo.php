...
class cCatalogo{
    ...
    function catalogo_ventafarmacia_filtro($alm_ven,$nom,$pactivo,$indicacion,$cod,$cat,$mar,$est,$limit){
        $sql="SELECT * 
	FROM tb_producto p
	INNER JOIN tb_categoria c ON p.tb_categoria_id=c.tb_categoria_id
	INNER JOIN tb_marca m ON p.tb_marca_id=m.tb_marca_id
	...
	AND tb_producto_est LIKE '%$est%' ";
        ...
        if($nom!="" && $indicacion==0 && $pactivo==0)$sql.=" AND (p.tb_producto_nom LIKE '%$nom%' 
        OR tb_producto_princactivo LIKE '%$nom%' OR p.tb_producto_indicacion LIKE '%$nom%')";
        if($pactivo==1)$sql.=" AND tb_producto_princactivo LIKE '%$nom%' ";
        if($indicacion==1)$sql.=" AND tb_producto_indicacion LIKE '%$nom%' ";
        if($cod!="")$sql.=" AND tb_presentacion_cod LIKE '%$cod%' ";
        if($cat!="")$sql.=" AND p.tb_categoria_id IN ($cat) ";
        if($mar!="")$sql.=" AND p.tb_marca_id=$mar ";
