...
function catalogo_filtro()
{
	$.ajax({
		...
		complete: function(){
            if ($('#hdd_tipo_tabla').val()==='1'){
                catalogo_tabla();
            }else{
                catalogo_iniciar_tabla();
            }
		}
	});
}
...
function catalogo_iniciar_tabla(){
    console.log('catalogo_inicio');
    $.ajax({
        type: "POST",
        url: $('#hdd_modo').val(),
        async:true,
...
function modo(url){
	...
	if(url=='catalogo_precio.php')
	{
		$('#chk_fil_unibas').attr("checked",true);
		$("#div_modo").html("Modo: Editar Precio");
		$("#div_fecha").hide(100);
	}
	...
	{
		$('#chk_fil_unibas').attr("checked",false);
		$("#div_modo").html("Modo: Inventario Valorizado x Almacén");
		$("#div_fecha").show(100);
	}
	
	if(url=='inventario_valor_emp.php')
	{
		$('#chk_fil_unibas').attr("checked",false);
		$("#div_modo").html("Modo: Inventario Valorizado x Empresa");
		$("#div_fecha").show(100);
	}

	if(url=='catalogo_impresion.php')
	{
		$('#chk_fil_unibas').attr("checked",false);
		$("#div_modo").html("Modo: Catálogo - Impresión");
		$("#div_fecha").hide(100);
	}
	...
	if(url=='inventario_stock_emp.php')
	{
		$('#chk_fil_unibas').attr("checked",true);
    ...
    <article class="content">
    	<div class="contenido">
                    ...
                    <tr>
                      ...
                      <form action="catalogo_reporte_xls.php" method="post" target="_blank" id="for_rep_xls">
						<input type="hidden" id="hdd_tabla" name="hdd_tabla" />
                          <input type="hidden" id="hdd_tipo_tabla" name="hdd_tipo_tabla"/>
                      </form>
                      </td>
            ...
            MODO VISTA
                <div id="menu">
                    <ul>
                        <?php /*?><li><a href="#" onClick="modo('catalogo_tabla.php')">Catálogo - Stock Valorizado</a></li>
                        <li><a href="#" onClick="modo('catalogo_impresion.php')">Catálogo Impresión</a></li>
                        <li>----------------------------</li><?php */?>
                        <li><a href="#" onClick="modo('catalogo_tabla_edit.php')">Editar Producto</a></li>
                        <li><a href="#" onClick="modo('catalogo_precio.php')">Editar Precios</a></li>
                        <li>----------------------------</li>
                        <li><a href="#" onClick="modo('catalogo_stock.php')">Ingresar Stock Inicial</a></li>
                        <li>----------------------------</li>
                        <li><a href="#" onClick="modo('catalogo_stock_rep.php')">Stock por Empresa</a></li>
                        <li>----------------------------</li>
                        <!--<li><a href="#" onClick="modo('inventario_tabla.php')" title="Inventario valorizado por almacén. (Promedio)">Invent. Valorizado: Prom x almacén</a></li>
                       ...
                       <?php /*?>
                       <li>----------------------------</li>
                       <li><a href="#" onClick="modo('inventario_valor_total.php')">Costo Prom. Tot. - Stock Empresas</a></li>
                       <?php */?>
                    </ul>
                </div>
            </div>
            <?php /*?>
            <div id="div_modo_reporte">
            REPORTES
                <div id="menu">
                    <ul>
                        <li><a href="#" onClick="catalogo_reporte('catalogo_reporte.php')">Stock Valorizado</a></li>
