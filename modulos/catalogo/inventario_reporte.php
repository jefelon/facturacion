...
//fecha inventario
$fecini='01-01-2013';
$fecfin=$_POST['txt_fil_inv_fec'];

if(isset($_POST['txt_fil_inv_fec']))
{
	$inventario_fecha=mostrarFecha($fecfin);
}
else
{
	$inventario_fecha=date('d-m-Y');
}
...
//____
	...
	$pdf->SetTextColor(0,49,159);
	$pdf->Cell(150,4,$inventario_fecha,0,1,'L');
	...
	*****************************************************/
	...
	if($num_rows>=1)
	{
		while($dt1 = mysql_fetch_array($dts1))
		{
			...
			//STOCK
						$stock_kardex=stock_kardex($dt1['tb_catalogo_id'],$_POST['cmb_fil_pro_alm'],fecha_mysql($fecini),fecha_mysql($fecfin),$_SESSION['empresa_id']);
						
						//COSTOS
						
						$costo_ponderado_array=costo_ponderado($dt1['tb_catalogo_id'],$_POST['cmb_fil_pro_alm'],fecha_mysql($fecini),fecha_mysql($fecfin),$stock_kardex,$dt1['tb_catalogo_precos'],$dt1['tb_catalogo_precosdol'],$_SESSION['empresa_id']);
		...
		//{
			...
			$data[1]['TEXT'] = utf8_decode($dt1['tb_marca_nom']);
			$data[2]['TEXT'] = utf8_decode($dt1['tb_categoria_nom']);
			
			$data[3]['TEXT'] = formato_money($costo_ponderado_dolares);
			$data[3]['T_ALIGN'] = 'R';
			
			$data[4]['TEXT'] = formato_money($costo_ponderado_soles);
			$data[4]['T_ALIGN'] = 'R';
			
			$data[5]['TEXT'] = formato_money($preven);
			$data[5]['T_ALIGN'] = 'R';
			
			$data[6]['TEXT'] = "".$stock_kardex;
			$data[6]['T_ALIGN'] = 'R';
			
			$data[7]['TEXT'] = formato_money($valorizado_dol);
			$data[7]['T_ALIGN'] = 'R';
			
			$data[8]['TEXT'] = formato_money($valorizado);
			$data[8]['T_ALIGN'] = 'R';
		...
		$data = Array();
			...
			$data[7]['TEXT'] = formato_money($total_valorizado_dol);
			$data[7]['T_SIZE'] = 7;
			$data[7]['T_ALIGN'] = 'R';
			$data[7]['V_ALIGN'] = "M";
			
			$data[8]['TEXT'] = formato_money($total_valorizado);
			$data[8]['T_SIZE'] = 7;
			$data[8]['T_ALIGN'] = 'R';
			$data[8]['V_ALIGN'] = "M";
