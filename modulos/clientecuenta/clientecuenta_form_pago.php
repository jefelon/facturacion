...
//TIPO DE VENTA
	...
	//arreglo para mostrar el nuevo saldo a pagar, (menos el pago realizado)
	$total_pagado=$total_pagado-$clicue_mon;
	
	//saldo a pagar
	$saldo_pagar=$cuenta_clicue_mon-$total_pagado;
		
	//datos para registro de pago
	$r_clicue_fec=$clicue_fec;
	//$r_forpag_id=1;
//	$r_modpag_id=1;
//	$r_fecven='00-00-0000';
//	$r_tar_id=0;
...
//	$r_numope="";
	$r_clicue_tip=2;//salida	
	$r_clicue_glo=$clicue_glo;
	$r_monto=$clicue_mon;//monto pagado
	$r_clicue_est=0;
//	$r_ver=1;
//	$r_clicue_idp=$clicue_idp;
...
/*	$tipo=2;
	$tipo_registro=2;
	$dts=$oClientecuenta->mostrar_por_venta($ven_id,$tipo,$tipo_registro);
	while($dt = mysql_fetch_array($dts)){
		$total_pagado+=$dt['tb_clientecuenta_mon'];
	}
	mysql_free_result($dts);
		...
		submitHandler: function() {
			$.ajax({
				...
				success: function(data){						
					...
					if(data.clicue_act=='imprime')
					{
						recibo_impresion(data.clicue_id);
					}
				},
				complete: function(){
    ...
    <table width="100%">
       ...
       <?php //if($_POST['action2']=='pago'){?>
        <tr>
          ...
          <td><?php echo $doc_abr.' '.$ven_numdoc?></td>
          <td align="right">Fecha Venta:</td>
          <td><?php echo $ven_fec?></td>
        </tr>
        <tr>
          <td align="right" valign="top">Total Venta:</td>
          <td colspan="3"><?php echo formato_money($ven_tot)?></td>
        </tr>
      </table>
</fieldset>
<fieldset><legend>Cuenta</legend>
      <?php echo $cuenta_clicue_glo?>
      <table>
        <tr>
          <td align="right" valign="top">Monto a Pagar:</td>
          <td colspan="3" align="right"><?php echo formato_money($cuenta_clicue_mon)?></td>
        </tr>
        <tr>
          ...
          <td colspan="3" align="right"><?php echo formato_money($total_pagado)?></td>
        </tr>
        <tr>
          <td align="right" valign="top">Saldo:</td>
          <td colspan="3" align="right"><?php echo formato_money($saldo_pagar)?></td>
        </tr>
      </table>
        <?php //}?>
</fieldset>
<fieldset><legend>Pago</legend>
        ...
        <tr>
            ...
            <td colspan="3"><input name="txt_clicue_mon" type="text" class="moneda_monto_" id="txt_clicue_mon" style="text-align:right" value="<?php echo $r_monto?>" size="10" <?php //if($_POST['action']=="editar_pago")echo 'readonly'?>/></td>
        </tr>
