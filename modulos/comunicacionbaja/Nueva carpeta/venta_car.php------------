...
if($_POST['action']=='agregar'){
	if($_POST['cat_can']>0){
		...
		if($num==0){
			...
			$_SESSION['venta_nom'][$_POST['cat_id']]=$pro_nom . ' ' . $tipo_item_txt;
				...
				if(($_POST['cat_id']!=$indice) and ($pre_id==$_SESSION['presentacion_id'][$indice])){
					...
					if($cat_mul>$_SESSION['catalogo_mul'][$indice])
					{
						...
						else
						{
							...
							unset($_SESSION['venta_nom'][$indice]);
...
if($_POST['action']=='agregar_servicio'){
	...
	$_SESSION['servicio_tip'][$_POST['ser_id']]		= $_POST['ser_tip'];
	...
	$_SESSION['servicio_nom'][$_POST['ser_id']]		= $_POST['ser_nom'] . ' ' . $tipo_item_txt;
}
...
if($_POST['action']=='quitar_servicio'){
	...
	unset($_SESSION['servicio_tip'][$_POST['ser_id']]);
	unset($_SESSION['servicio_nom'][$_POST['ser_id']]);
}
        ...
        <table cellspacing="1" id="tabla_venta_car" class="tablesorter">
			...
			if($num_rows>0)foreach($_SESSION['venta_car'] as $indice=>$cantidad){			
				...
				?>
                        <tr>
                          ...
                          <td><?php echo $_SESSION['venta_nom'][$indice] ?></td>
            ...
            <?php 
            	if($num_rows2>0)foreach($_SESSION['servicio_car'] as $indice=>$cantidad){
					...
					//tipo g/e/i ingresado
					$tipo_item	=$_SESSION['servicio_tip'][$indice];
					switch ($tipo_item) {
						case '1':
							$tipo_item_txt = "";
							break;
                	...
                	<tr>
                	  ...
                	  <td>Gravado</td>
                	  <td>&nbsp;</td>
                    	<td><?php echo $_SESSION['servicio_nom'][$indice] ?></td>
                    	<td>UN</td>
                    	<td align="right"><?php echo $cantidad?></td>
                    	<td align="right"><?php echo formato_money($precio_unitario)?></td>
                        <td align="right">
