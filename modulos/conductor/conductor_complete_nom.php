...
class ElementoAutocompletar {
   ...
   var $licencia;
   var $categoria;
   function __construct($id, $label, $value, $direccion, $documento,$licencia,$categoria){
	  $this->id = $id;
      $this->label = $label;
      $this->value = $value;
...
while ($fila = mysql_fetch_array($rs)){
   array_push($arrayElementos, new ElementoAutocompletar($fila["tb_conductor_id"], $fila["tb_conductor_doc"].'-'.$fila["tb_conductor_nom"],$fila["tb_conductor_nom"], $fila['tb_conductor_dir'], $fila['tb_conductor_doc'], $fila['tb_conductor_lic'], $fila['tb_conductor_cat']));
}
