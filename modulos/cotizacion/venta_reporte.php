...
$dts1=$oCotizacion->mostrar_filtro(fecha_mysql($_POST['txt_fil_ven_fec1']),fecha_mysql($_POST['txt_fil_ven_fec2']),$_POST['cmb_fil_ven_doc'],$_POST['hdd_fil_cli_id'],$_POST['cmb_fil_ven_est'],$_SESSION['usuario_id'],$_SESSION['puntoventa_id'],$_POST['chk_fil_ven_may']);
	...
	*****************************************************/
	...
	if($num_rows>=1)
	{
		while($dt1 = mysql_fetch_array($dts1))
		{
			...
			$venmay="";
			if($dt1['tb_venta_may']==1)$venmay="VM - ";
		...
		//{
			...
			//DETALLE DE VENTA
			$dts2=$oCotizacion->mostrar_venta_detalle_ps($dt1['tb_venta_id']);
			...
			if($num_rows_vp>0){
				while($rw1 = mysql_fetch_array($rws1)){
					...
					$texto_pago2[]=$forma.' '.$modo.': '.$pago_mon;
					
				}
                mysql_free_result($rws1);
			}
			
			if($num_rows_vp==1)$texto_venta_pago='* '.$texto_pago1[0];
			
			if($num_rows_vp>1){
				foreach($texto_pago2 as $indice=>$valor){
					$texto_venta_pago.='* '.$valor.'  ';
				}
			}
	...
	//$cat_id=40;
		...
		$dts1=$oCotizacion->mostrar_filtro_detalle( fecha_mysql($_POST['txt_fil_ven_fec1']),fecha_mysql($_POST['txt_fil_ven_fec2']),$_POST['cmb_fil_ven_doc'],$tipo[$indice],$cat_ids,$_POST['hdd_fil_cli_id'],$_POST['cmb_fil_ven_est'],$_SESSION['usuario_id'],$_SESSION['puntoventa_id'],$_POST['chk_fil_ven_may']);
		$num_rows= mysql_num_rows($dts1);
