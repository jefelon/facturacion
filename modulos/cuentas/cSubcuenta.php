...
class cSubcuenta{
	...
	$sql="SELECT * FROM tb_subcuenta ORDER BY tb_subcuenta_des";
	$oCado = new Cado();
	$rst=$oCado->ejecute_sql($sql);
	return $rst;
	}
	function mostrarTodos_cue_oby($id1,$oby){
	$sql="SELECT * FROM tb_subcuenta 
	...
	$sql="SELECT * FROM tb_subcuenta WHERE tb_cuenta_id=$id1";
	$oCado = new Cado();
	$rst=$oCado->ejecute_sql($sql);
	return $rst;
	}
...
WHERE tb_subcuenta_id =$id";
	...
	$sql="DELETE FROM tb_subcuenta WHERE tb_subcuenta_id=$id";
	$oCado = new Cado();
	$rst=$oCado->ejecute_sql($sql);
	return $rst;
	}
