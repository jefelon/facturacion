...
//FECHA
$texto_fecha=$_POST['txt_fil_egr_fec1']." AL ".$_POST['txt_fil_egr_fec2'];
...
//proveedor
if($_POST['hdd_fil_pro_id']>0)
{
	$dts=$oProveedor->mostrarUno($_POST['hdd_fil_pro_id']);
	$dt = mysql_fetch_array($dts);
	...
	$texto_proveedor=$pro_doc.' '.$pro_nom;
}

if($_POST['cmb_fil_egr_est']==1)$texto_estado='CANCELADO';
if($_POST['cmb_fil_egr_est']==2)$texto_estado='EMITIDO';
...
//____
	...
	if($_POST['cmb_fil_caj_id']>0){
	$pdf->SetTextColor(118,0,3);
	$pdf->Cell(25,4,'CAJA:',0,0,'L');
	$pdf->SetTextColor(0,49,159);
	$pdf->Cell(150,4,utf8_decode($caj_nom),0,1,'L');
	}

	$pdf->SetTextColor(118,0,3);
	$pdf->Cell(25,4,'FECHA:',0,0,'L');
	$pdf->SetTextColor(0,49,159);
	$pdf->Cell(150,4,utf8_decode($texto_fecha),0,1,'L');

	if($_POST['hdd_fil_pro_id']>0){
	$pdf->SetTextColor(118,0,3);
	$pdf->Cell(25,4,'ANEXO:',0,0,'L');
	$pdf->SetTextColor(0,49,159);
	...
	$pdf->SetTextColor(0,49,159);
	$pdf->Cell(150,4,utf8_decode($_POST['txt_fil_egr_doc']),0,1,'L');*/

	if($_POST['cmb_fil_egr_est']>0){
	$pdf->SetTextColor(118,0,3);
	$pdf->Cell(25,4,'ESTADO:',0,0,'L');
	$pdf->SetTextColor(0,49,159);
	$pdf->Cell(150,4,utf8_decode($texto_estado),0,1,'L');
	}
	...
	*****************************************************/
	...
	if (isset($bTableSplitMode)) $pdf->tbSetSplitMode($bTableSplitMode);
	...
	if($num_rows>=1)
	{
		while($dt1 = mysql_fetch_array($dts1))
		{
			...
			$data[1]['TEXT'] = $dt1['tb_egreso_id'];
			$data[2]['TEXT'] = $documento;
			$data[3]['TEXT'] = utf8_decode($dt1['tb_proveedor_doc'].' '.$dt1['tb_proveedor_nom']);
			$data[4]['TEXT'] = utf8_decode($dt1['tb_egreso_det']);
			$data[5]['TEXT'] = utf8_decode($dt1['tb_cuenta_des']);
		
			$data[6]['TEXT'] = $dt1['tb_subcuenta_des'];
			$data[6]['T_ALIGN'] = 'L';
			
			$data[7]['TEXT'] = formato_money($dt1['tb_egreso_imp']);
			$data[7]['T_ALIGN'] = 'R';
			
			$data[8]['TEXT'] = utf8_decode($estado);
			
			$data[9]['TEXT'] = utf8_decode($dt1['tb_caja_nom']);
		...
		$data = Array();
			...
			$data[7]['TEXT'] = formato_money($sum_imp);
			$data[7]['T_SIZE'] = 7;
			$data[7]['T_ALIGN'] = 'R';
			$data[7]['V_ALIGN'] = "M";
...
/*	
	...
	$pdf->MultiCellTag(100, 7, $sTxt,0,'L');
*/	
	...
	$nombre_archivo='rep_egresos_'.$fecha_actual.'.pdf';
