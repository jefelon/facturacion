...
class cGasto{
	...
	VALUES (
	NOW( ) ,NOW( ) ,  '$fec',  '$doc',  '$des',  '$imp',  '$modpag',  '$numope',  '$est',  '$cue',  '$subcue',  '$pro',  '$entfin_id',  '$caj_id', '$mon_id', '$tra_id', '$com_id', '$emp_id',  '$usu_id_reg', '$usu_id_mod'
	);"; 
	$oCado = new Cado();
	$rst=$oCado->ejecute_sql($sql);
	return $rst;	
	}
	function modificar($id,$fec,$doc,$des,$imp,$modpag,$numope,$est,$cue,$subcue,$pro,$entfin,$caj_id,$mon_id,$usu_id_mod){
	$sql = "UPDATE tb_gasto_r SET
	`tb_gasto_fecmod` = NOW( ) ,
	`tb_gasto_fec` =  '$fec',
	`tb_gasto_doc` =  '$doc',
	`tb_gasto_des` =  '$des',
	...
	$sql="SELECT DISTINCT(tb_gasto_des) FROM tb_gasto_r
	WHERE tb_empresa_id=$emp
...
WHERE tb_gasto_id=$id";
	...
	$sql="SELECT * 
	FROM tb_gasto_r g
	INNER JOIN tb_cuenta_r c ON g.tb_cuenta_id = c.tb_cuenta_id
	INNER JOIN tb_caja_r cj ON g.tb_caja_id = cj.tb_caja_id
	...
	LEFT JOIN tb_subcuenta_r sc ON g.tb_subcuenta_id = sc.tb_subcuenta_id
	...
	function mostrar_filtro_fec($emp_id,$fec1,$fec2,$cue_id,$subcue_id,$pro_id,$entfin_id,$est,$caj_id,$mon_id){
	$sql="SELECT * 
	FROM tb_gasto_r g
	INNER JOIN tb_cuenta_r c ON g.tb_cuenta_id = c.tb_cuenta_id
	INNER JOIN tb_caja_r cj ON g.tb_caja_id = cj.tb_caja_id
	...
	function mostrar_por_compra($com_id){
	$sql="SELECT * 
	FROM tb_gasto_r g
	INNER JOIN tb_cuenta_r c ON g.tb_cuenta_id = c.tb_cuenta_id
	INNER JOIN tb_caja_r cj ON g.tb_caja_id = cj.tb_caja_id
	...
	function mostrar_suma($emp_id,$fec1,$fec2,$cue_id,$subcue_id,$pro_id,$entfin_id,$est,$caj_id){
	...
	$sql="DELETE FROM tb_gasto_r WHERE tb_gasto_id=$id";
	$oCado = new Cado();
	$rst=$oCado->ejecute_sql($sql);
	return $rst;
	}
