...
//FECHA
$texto_fecha=$_POST['txt_fil_ing_fec1']." AL ".$_POST['txt_fil_ing_fec2'];
...
if($_POST['hdd_fil_cli_id']>0)
{
	...
	$texto_cliente=$cli_doc.' '.$cli_nom;
}

if($_POST['cmb_fil_ing_est']==1)$texto_estado='CANCELADO';
if($_POST['cmb_fil_ing_est']==2)$texto_estado='EMITIDO';
	...
	*****************************************************/
	...
	if($num_rows>=1)
	{
		while($dt1 = mysql_fetch_array($dts1))
		{
			...
			$data[1]['TEXT'] = $dt1['tb_ingreso_id'];
			$data[2]['TEXT'] = $documento;
			$data[3]['TEXT'] = utf8_decode($dt1['tb_cliente_doc'].' '.$dt1['tb_cliente_nom']);
			$data[4]['TEXT'] = utf8_decode($dt1['tb_ingreso_det']);
			$data[5]['TEXT'] = utf8_decode($dt1['tb_cuenta_des']);
		
			$data[6]['TEXT'] = $dt1['tb_subcuenta_des'];
			$data[6]['T_ALIGN'] = 'L';
			
			$data[7]['TEXT'] = formato_money($dt1['tb_ingreso_imp']);
			$data[7]['T_ALIGN'] = 'R';
