...
{	
	$.ajax({
		...
		beforeSend: function() {
			$('#cmb_fil_ref_id').html('<option value="">Cargando...</option>');
        },
		success: function(html){
			$('#cmb_fil_ref_id').html(html);
		}
	});
}
...
$(function() {
	...
	cmb_fil_cue_id();
	cmb_fil_caj_id();
	cmb_fil_ref_id('<?php echo $ref_id?>');
...
<form name="for_fil_ing" id="for_fil_ing" target="_blank" action="<?php echo $_POST['act']?>" method="post">
<fieldset><legend>Filtro de Ingresos</legend>
    ...
    <label for="txt_fil_ing_doc">Documento:</label>
    <input name="txt_fil_ing_doc" type="text" id="txt_fil_ing_doc" size="15" maxlength="15">
    <br>
    ...
    <label for="cmb_fil_ref_id">Referencia: BANCO</label>
    <!--<select name="cmb_fil_ref_id" id="cmb_fil_ref_id">
    </select>-->
    <input name="cmb_fil_ref_id" type="hidden" value="2">
