...
while ($fila = mysql_fetch_array($rs)){
    ...
    array_push($arrayElementos, new ElementoAutocompletar($fila["tb_lote_id"], $fila["tb_lote_numero"].' - FV: '.$fila["tb_lote_fechavence"] .' - Stock: '.$fila['tb_lote_exisact']  .' - FCompra:'. mostrarFecha($lt["tb_compra_fec"]),$fila["tb_lote_numero"], mostrarFecha($fila["tb_lote_fechafab"]),mostrarFecha($fila["tb_lote_fechavence"]), $fila['tb_lote_exisact']));
}
