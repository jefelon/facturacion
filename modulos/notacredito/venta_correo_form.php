...
$dts= $oNotacredito->mostrarUno($_POST['ven_id']);
...
if($_POST['action']=='enviar')
{
	...
	$asu="$doc_nom ELECTRONICA $ser-$num | $empresa";
	...
	$mentip.="<p><strong>$doc_nom ELECTRONICA $ser-$num</strong></p>";

	if($doc_nom=='NOTA DE CREDITO')$mentip.='<a style="color: blue;" target="_blank" href="http://www.granadosllantas.com/facturacion/recursos/notacredito/notacredito_cpeimp.php?id_doc='.md5($key.$_POST['ven_id']).'">Descargar PDF</a><br/>';

	$mentip.='<a style="color: blue;" target="_blank" href="http://www.granadosllantas.com/facturacion/recursos/notacredito/descargar_xml.php?id_doc='.md5($key.$_POST['ven_id']).'">Descargar XML</a><br/>';
	...
	$("#for_ven_cor").validate({
		submitHandler: function() {
			...
			if(confirm("Confirmar envío de correo?"))
			{
				...
				$.ajax({
					type: "POST",
					url: "../notacredito/venta_correo_reg.php",
					async:true,
					dataType: "json",
					data: fd,
					cache: false,
...
<input type="hidden" id="hdd_cli_nom" name="hdd_cli_nom" value="<?php echo $cli_nom?>" />
  ...
  <table border="0" cellspacing="0" cellpadding="1" width="100%">
    ...
    <tr>
      <td align="left"><label for="txt_cli_emacop">Con Copia:</label></td>
