...
if($_POST['action']=='enviar')
{
	...
	if($doc_nom=='NOTA DE DEBITO')$mentip.='<a style="color: blue;" target="_blank" href="http://www.granadosllantas.com/facturacion/recursos/notadebito/notadebito_cpeimp.php?id_doc='.md5($key.$_POST['ven_id']).'">Descargar PDF</a><br/>';

	$mentip.='<a style="color: blue;" target="_blank" href="http://www.granadosllantas.com/facturacion/recursos/notadebito/descargar_xml.php?id_doc='.md5($key.$_POST['ven_id']).'">Descargar XML</a><br/>';
	...
	$("#for_ven_cor").validate({
		submitHandler: function() {
			...
			if(confirm("Confirmar envío de correo?"))
			{
				...
				$.ajax({
					type: "POST",
					url: "../notadebito/venta_correo_reg.php",
					async:true,
					dataType: "json",
					data: fd,
					cache: false,
