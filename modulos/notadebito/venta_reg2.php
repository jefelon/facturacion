...
require_once("cNotadebito.php");
$oNotadebito = new cNotadebito();
require_once ("../talonariond/cTalonario.php");
...
if($_POST['action_venta']=="insertar")
{
	if(!empty($_POST['txt_ven_fec']))
	{
		...
		if($tipo_documento=='F' OR $tipo_documento=='B')
		{
			...
			$dts= $oNotadebito->verificar_venta($serie,$correlativo);
			...
			if($ven_id>0)
			{
				...
				$est="CANCELADA";
				$documento_tipdoc=4;//notadebito

				$total=moneda_mysql($_POST['txt_ven_mon']);
				$gravado=moneda_mysql(formato_money($total/1.18));
				$igv=moneda_mysql(formato_money($gravado*0.18));


				//insertamos venta
				$oNotadebito->insertar( 
				...
				//ultimo
					$dts=$oNotadebito->ultimoInsert();
					$dt = mysql_fetch_array($dts);
				$notdeb_id=$dt['last_insert_id()'];
					mysql_free_result($dts);
				...
				// {
					...
					$tipounidadmedida_id=13;
					$nro=1;

					$oNotadebito->insertar_detalle(
						$tipven,
						$catalogo_id,
						$servicio_id,
						$detalle,
						...
						$igv,
						$notdeb_id,
						$tipoafectacionigv_id,
						$tipounidadmedida_id,
						$tiposistemacalculoisc_id,
						$isc,
						$nro
					);
    ...
    //             mysql_free_result($dts2);
				...
				$data['ven_msj']='Se registró Nota de Débito correctamente.'.$numdoc;
			}
			else
			{
