...
function venta_filtro()
{
	$.ajax({
		type: "POST",
		url: "../notadebito/venta_filtro.php",
		async:true,
		dataType: "html",                      
		//data: ({
...
function venta_form2(act,idf){
	$.ajax({
		type: "POST",
		url: "../notadebito/venta_form2.php",
		async:true,
		dataType: "html",                      
		data: ({
			action: act,
...
function venta_impresion(idf){
	$.ajax({
		type: "POST",
		url: "../notadebito/venta_preimpresion.php",
		async:true,
		dataType: "html",                      
		data: ({
			ven_id:	idf
...
{      
	...
	//{
		$.ajax({
			type: "POST",
			url: "../notadebito/enviar_sunat.php",
			async:true,
			dataType: "json",
			data: ({
				ven_id:		id
...
{      
	if(confirm("Realmente desea anular venta "+texto+", se actualizará el stock. ASEGURESE QUE LAS CANTIDADES DE PRODUCTO SE PUEDAN REPONER CORRECTAMENTE.  Continuar?")){
		$.ajax({
			type: "POST",
			url: "../notadebito/venta_anular.php",
			async:true,
			dataType: "json",
			data: ({
				action: "anular",
...
function venta_correo_form(act,venid){
	...
	//{
		$.ajax({
			type: "POST",
			url: "../notadebito/venta_correo_form.php",
			async:true,
			dataType: "html",
			data: ({
				action: act,
...
function venta_correo_email(ven_id){
	$.ajax({
		type: "POST",
		url: "../notadebito/venta_correo_email.php",
		async:true,
		dataType: "html",
		data: ({
			ven_id:	ven_id
...
$(function() {
	...
	$( "#div_venta_form" ).dialog({
		title:'Información de Nota de Débito',
		autoOpen: false,
		resizable: false,
		height: 600,
		width: 980,
    ...
    <article class="content">
    	<div class="contenido">
            ...
            <table align="center" class="tabla_cont">
                  <tr>
                    <td class="caption_cont">NOTAS DE DÉBITO</td>
                  </tr>
                  <tr>
