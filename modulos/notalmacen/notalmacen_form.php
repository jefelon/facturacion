...
function txt_notalm_numdoc(){	
	$.ajax({
		type: "POST",
		url: "../notalmacen/notalmacen_txt_numdoc.php",
		async:false,
		dataType: "json",                      
		data: ({
			alm_id: $('#cmb_alm_id').val(),
		...
		beforeSend: function() {
			$('#txt_notalm_numdoc').val('Cargando...');			
        },
		success: function(data){	
			$('#txt_notalm_numdoc').val(data.numero);
			if(data.msj!="")
			{
...
$(function() {
	...
	$("#txt_notalm_numdoc").addClass("ui-state-active");
			...
			$.ajax({
				...
				success: function(data){
					if(data.redireccionar){
					 	alert("Nota de Almacén No Registrada.\n Por Favor Inicie Sesión Nuevamente.");
...
<fieldset>
  ...
  <tr>
    ...
    <td align="right" valign="top"><label for="txt_notalm_numdoc">N° Documento:</label></td>
    <td valign="top"><input name="txt_notalm_numdoc" type="text" id="txt_notalm_numdoc" size="12" maxlength="12" readonly value="<?php echo $cod?>" style="text-align:right; font-size:10pt;">
  ...
  <tr>
    <?php /*?><td align="right" valign="top"><label for="cmb_doc_id">Documento:</label></td>
    <td valign="top"><select name="cmb_doc_id" id="cmb_doc_id" <?php if($_POST['action']=='editar')echo 'disabled'?>>
    </select></td><?php */?>
    <td rowspan="2" align="right" valign="top"><label for="txt_notalm_des">Observación:</label></td>
