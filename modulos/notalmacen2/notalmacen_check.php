...
//{
	...
	{			
		...
		$dts=$oCatalogoproducto->presentacion_catalogo_stock_almacen($indice,$almacen);
		...
		$stock_notalmacen=$cantidad*$cat_mul;
		
				
		$num=0;
		foreach($_SESSION['notalmacen_car'] as $i=>$c){
		...
		if($num==0)
		{
			if($stock_notalmacen>$sto_num)
			{
				$dif=$stock_notalmacen-$sto_num;
		...
		if($num==1)
		{
			foreach($_SESSION['notalmacen_car'] as $i=>$c){
				...
				{
					$t1=$cat_mul*$cantidad;
					$t2=$_SESSION['catalogo_mul'][$i]*$_SESSION['notalmacen_car'][$i];
