...
if($_POST['action']=="editar"){
	$can 	= $_SESSION['traspaso_car'][$_POST['cat_id']];
...
$(function() {
	$("#for_item").validate({
		submitHandler: function() {
			$.ajax({
				type: "POST",
				url: "../traspaso/traspaso_car_item_reg.php",
				async:true,
				dataType: "json",
				data: $("#for_item").serialize(),
				beforeSend: function(){
					$('#div_item_form').dialog("close");
					$('#msj_traspaso_car_item').html("Cargando...");
					$('#msj_traspaso_car_item').show(100);				
				},
				success: function(data){
					$('#msj_traspaso_car_item').html(data.ite_msj);
				},
				complete: function(){
					traspaso_car('actualizar');			
				}
			});			
		},
		rules: {
