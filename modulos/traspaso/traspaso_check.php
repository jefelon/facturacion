...
//{
	if(isset($_SESSION['traspaso_car']))foreach($_SESSION['traspaso_car'] as $indice=>$cantidad)
	{			
		//presentacion catalogo
		$dts=$oCatalogoproducto->presentacion_catalogo_stock_almacen($indice,$almacen_traspaso);
		...
		$stock_traspaso=$cantidad*$cat_mul;
		
				
		$num=0;
		foreach($_SESSION['traspaso_car'] as $i=>$c){
			if(($indice!=$i) and ($pre_id==$_SESSION['presentacion_id'][$i]))
			{
				$num++;
			}
		}
		...
		if($num==0)
		{
			if($stock_traspaso>$sto_num)
			{
				$dif=$stock_traspaso-$sto_num;
		...
		if($num==1)
		{
			foreach($_SESSION['traspaso_car'] as $i=>$c){
				...
				{
					$t1=$cat_mul*$cantidad;
					$t2=$_SESSION['catalogo_mul'][$i]*$_SESSION['traspaso_car'][$i];
