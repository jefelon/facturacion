...
function tabsUsuario()
{
	...
	{ 
		...
		case 'editar':
			$("#tabs").tabs({
			disabled:[],
			selected: <?php if($_POST['ntab']>0){echo $_POST['ntab'];}else{echo '0';}?>
			});
		break 
		//default: 
...
{	
	...
	$('#cmb_emp_id').attr('disabled', 'disabled');
}
...
{	
	$.ajax({
		type: "POST",
		url: "../horario/cmb_hor_id.php",
		async:true,
		dataType: "html",                      
		data: ({
			hor_id: "<?php echo $hor_id?>"
		}),
		beforeSend: function() {
			$('#cmb_hor_id').html('<option value="">Cargando...</option>');
        },
		success: function(html){
			$('#cmb_hor_id').html(html);
		}
	});
}
...
{	
	$.ajax({
		...
		beforeSend: function() {
			$('#div_usuario_direccion_tabla').addClass("ui-state-disabled");
        },
		success: function(html){
			$('#div_usuario_direccion_tabla').html(html);
		},
		complete: function(){			
			$('#div_usuario_direccion_tabla').removeClass("ui-state-disabled");
		}
	});       
}
...
{	
	$.ajax({
		...
		beforeSend: function() {
			$('#div_usuario_telefono_tabla').addClass("ui-state-disabled");
        },
		success: function(html){
			$('#div_usuario_telefono_tabla').html(html);
		},
		complete: function(){			
			$('#div_usuario_telefono_tabla').removeClass("ui-state-disabled");
		}
	});       
}
...
function horario_form(act){
	if($('#cmb_hor_id').val()>0)
	{
		$.ajax({
			type: "POST",
			url: "../horario/horario_form.php",
			async:true,
			dataType: "html",                      
			data: ({
				action: act,
				hor_id:	$('#cmb_hor_id').val()
			}),
			beforeSend: function() {
...
<form id="for_usu">
    ...
    <td>
    <select name="cmb_usugru" id="cmb_usugru">
    </select>
    </td>
    </tr>
    <tr>
