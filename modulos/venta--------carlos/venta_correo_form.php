...
if($_POST['action']=='enviar')
{
	...
	$mentip="<p>Sírvase presionar el link imprimir para descargar los documentos, con las que podrá cancelar sus tributos.<br>Cualquier consulta, conversamos.</p>";
	$mentip.='<a href="/recursos/venta/venta_cpeimp_factura.php?id_factura='.md5($key.$_POST['venta_id']).'">Descargar PDF</a><br/>';
	$mentip.='<a href="/recursos/venta/descargar_xml.php?id_factura='.md5($key.$_POST['venta_id']).'">Descargar XML</a><br/>';
		...
		submitHandler: function() {
			...
			$.ajax({
				...
				success: function(data){
					$('#msj_venta').html(data.guipagnot_cor_msj);
				},
				complete: function(){
					venta_tabla();
				}
...
<input type="hidden" id="hdd_ven_id" name="hdd_ven_id" value="<?php echo $_POST['venta_id']?>" />
