...
if($_POST['action']=="insertar")
{
    ...
    $lts=$oLugar->validarViajeHorario($_POST['cmb_salida'],$_POST['cmb_llegada'], fecha_mysql($_POST['txt_fech_salida']),formato_hora_min_seg($_POST['txt_hora']),$_POST['cmb_vehiculo']);
    ...
    if($existe==true){
        $data['ven_ho_msj']="<pan style='background: red !important;color:#fff'>Ya existe un horario para el mismo carro y hora, recargue la pantalla.</pan>";
        echo json_encode($data);
    }
    else{
        if(!empty($_POST['cmb_salida']) && !empty($_POST['cmb_llegada']) && !empty($_POST['txt_fech_salida']) && !empty($_POST['txt_hora']) && !empty($_POST['cmb_vehiculo'])) {
            ...
            $oLugar->insertarViajeHorario(
                $_POST['cmb_salida'],
                $_POST['cmb_llegada'],
                fechahora_mysql($_POST['txt_fech_salida'].' '.$_POST['txt_hora']),
                $_POST['txt_hora'],
                $_POST['cmb_vehiculo'],
                $_POST['cmb_conductor'],
...
if($_POST['action']=="actualizar-vehiculo")
{
    if(!empty($_POST['hdd_vi_ho']) && !empty($_POST['cmb_vehiculo'])) {
        $lgs=$oLugar->modificar_vh_vehiculo(
            $_POST['hdd_vi_ho'],
            $_POST['cmb_vehiculo'],
            $_POST['cmb_conductor'],
            $_POST['cmb_copiloto'],
            fecha_mysql($_POST['txt_fech_salida']),
            $_POST['txt_hora']
        );
