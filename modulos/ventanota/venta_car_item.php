...
if($_POST['action']=="editar_producto"){
	...
	$catalogo_id=$_SESSION['ventanota_car'][$_POST['ite_id']];
	...
	$can 	= $_SESSION['ventanota_can'][$_POST['ite_id']];
	$preven = $_SESSION['ventanota_preven'][$_POST['ite_id']];	
	
	//Verifico si el descuento realizado es de tipo porcentaje o en dinero 1% - 2S/.
	$tipdes = $_SESSION['ventanota_tipdes'][$_POST['ite_id']];				
	if($tipdes == 1){
		$des = ($_SESSION['ventanota_des'][$_POST['ite_id']]);//Descuento Venta Detalle
	}
	if($tipdes == 2){
		$des = $_SESSION['ventanota_des'][$_POST['ite_id']];//Descuento Venta Detalle	
	}
...
if($_POST['action']=="editar_servicio"){
	$servicio_id=$_SESSION['vn_servicio_car'][$_POST['ite_id']];
	
	$dts=$oServicio->mostrarUno($servicio_id);
	$dt = mysql_fetch_array($dts);
	...
	$can 	= $_SESSION['vn_servicio_can'][$_POST['ite_id']];
	$preven = $_SESSION['vn_servicio_preven'][$_POST['ite_id']];	
	
	//Verifico si el descuento realizado es de tipo porcentaje o en dinero 1% - 2S/.
	$tipdes = $_SESSION['vn_servicio_tipdes'][$_POST['ite_id']];				
	if($tipdes == 1){
		$des = ($_SESSION['vn_servicio_des'][$_POST['ite_id']]);//Descuento Venta Detalle
	}
	if($tipdes == 2){
		$des = $_SESSION['vn_servicio_des'][$_POST['ite_id']];//Descuento Venta Detalle	
	}
...
$(function() {
	$("#for_item").validate({
		submitHandler: function() {
			$.ajax({
				type: "POST",
				url: "../ventanota/venta_car_item_reg.php",
				async:true,
				dataType: "json",
...
<input name="hdd_item_id" id="hdd_item_id" type="hidden" value="<?php echo $_POST['ite_id']?>">
    <table>
    	<tr>
    	  <td>Stock:</td>
