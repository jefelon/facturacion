...
if($_POST['action']=='agregar'){
	if($_POST['cat_can']>0){
		...
		if($num==0){
			...
			//NOMBRE
			$_SESSION['venta_nom'][$_POST['cat_id']]=$_POST['cat_nom'];
			//TIPO DE DESCUENTO  1 PORCENTAJE	2 SOLES		
			$_SESSION['venta_tipdes'][$_POST['cat_id']]=$_POST['cat_tipdes'];
			
			//DESCUENTO
			$_SESSION['venta_des'][$_POST['cat_id']]=moneda_mysql($_POST['cat_des']*$_POST['cat_can']);

			//GRAVADO/EXONERADO/INAFECTO
			$_SESSION['venta_tip'][$_POST['cat_id']]=$_POST['cat_tip'];
				...
				if(($_POST['cat_id']!=$indice) and ($pre_id==$_SESSION['presentacion_id'][$indice])){
					...
					if($cat_mul>$_SESSION['catalogo_mul'][$indice])
					{
						...
						else
						{
							...
							unset($_SESSION['venta_tip'][$indice]);
					...
					else
					{
						...
						if($st_res>0)
						{
							...
							$_SESSION['venta_nom'][$_POST['cat_id']]=$_POST['cat_nom'];//id cat - nombre
							...
							$_SESSION['venta_tip'][$_POST['cat_id']]=$_POST['cat_tip'];//id cat - descuento						
							//$_SESSION['venta_igv'][$_POST['cat_id']]=$_POST['cat_igv'];//id cat - igv
...
function calcular_vuelto(){
	...
	$.ajax({
		type: "POST",
		url: "../../modulos/venta/venta_vuelto.php",
		async:true,
		dataType: "html",                      
		data: ({
			importe: importe,				
...
?>
        ...
        <table cellspacing="1" id="tabla_venta_car" class="tablesorter">
			...
			if($num_rows>0)foreach($_SESSION['venta_car'] as $indice=>$cantidad){			
				...
				$tipo_item	=$_SESSION['venta_tip'][$indice];
				switch ($tipo_item) {
					case '1':
						$tipo_item_txt = "";
						break;
				...
				?>
                        <tr>
                          ...
                          <td><?php echo $dt1['tb_producto_nom'].' '.$tipo_item_txt?></td>
...
<table border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td nowrap="nowrap"><label for="txt_ven_opegra" style="font-size:12px;margin-right: 10px;"><strong>OPERACIONES GRATUITAS:</strong></label></td>
    <td align="right"><input name="txt_ven_opegra" type="text" id="txt_ven_opegra" style="text-align:right; font-size:14px" value="<?php echo formato_money($ope_gratuitas)?>" size="15" readonly></td>
  </tr>
  <tr>
